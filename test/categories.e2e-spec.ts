...
describe('CategoriesController (e2e)', () => {
  ...
  const COMPLEMENT_URL = '/categories';
  ...
  describe('create - /accounts/{:idAccount}/categories (POST)', () => {
    ...
    it('should return category created', async () => {
      await request(app.getHttpServer())
        ...
        .expect(201)
        .then((res) => {
          expect(res.body.category.name.toLowerCase()).toBe(
            mockToCreateCategory.name.toLowerCase(),
          );
  ...
  describe('findAll - /accounts/{:idAccount}/categories (GET)', () => {
      ...
      await request(app.getHttpServer())
        ...
        .then((res) => {
          expect(res.body.categories).toHaveLength(
            accountTest1.categories.length,
          );
        });
    });
    ...
    it('should return all categories except those for which "isActive" is false', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .then((res) => {
          expect(res.body.categories).toHaveLength(
            accountTest1.categories.length - 1,
          );
        });
    });
  });
  ...
  describe('update - /accounts/{:idAccount}/categories/{:id} (PATCH)', () => {
    ...
    it('should return a updated category', async () => {
      await request(app.getHttpServer())
        ...
        .expect(200)
        .then(({ body }) => {
          expect(body.category).toMatchObject(mockToUpdateCategory);
          expect(body.category.account.id).toBe(accountTest1.id);
          expect(typeof body.category.id).toBe('string');
          expect(body.category.isActive).toBeTruthy();
        });
