...
export class CreditPaymentsService {
  ...
  async findAll(idAccount: string, onlyPendingDto: OnlyPendingDto) {
    try {
      const qb =
        this.creditPaymentRepository.createQueryBuilder('credit_payment');
      const credit_payments = await qb
        ...
        .leftJoinAndSelect(
          'credit_payment_expenses.category',
          'credit_payment_expenses_category',
        )
        .leftJoinAndSelect(
          'credit_payment_expenses.subcategory',
          'credit_payment_expenses_subcategory',
        )
        .leftJoinAndSelect(
          'credit_payment_expenses.user',
          'credit_payment_expenses_user',
        )
        .getMany();
  ...
  async findOne(id: string) {
    try {
      const credit_payment = await this.creditPaymentRepository.findOneBy({
        id,
      });
  ...
  async update(
    id: string,
    updateCreditPaymentDto: UpdateCreditPaymentDto,
    user: User,
    idAccount: string,
  ) {
