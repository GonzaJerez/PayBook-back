...
import {adminTest,user1,user2,user3,account1,account2,account3, category1, category2, category3, subcategory1, subcategory2, subcategory3, expense1, expense2, expense3, expense4, subcategory4} from './mocks/seedMock';
...
export class SeedService {
  ...
  private createAccountsForEachUser(users:User[]):User[]{
    return users.map((user) =>{
      ...
      const accountsWithCategories = this.createCategoriesForEachAccount(accounts, expenses)
  ...
  private createCategoriesForEachAccount(accounts:Account[], expenses:Expense[]):Account[]{
    ...
    return accounts.map( (account,idx) => {
      ...
      const categories = this.categoryRepository.create([
        ...
        if(idx === 0){
          ...
          categoriesWithSubcategories = this.createSubategoriesForEachCategory(categories, filteredExpenses)
        } else {
          filteredExpenses = expenses.filter((exp, index) => index >= 4)
          categoriesWithSubcategories = this.createSubategoriesForEachCategory(categories, filteredExpenses)
        }
  ...
  private createSubategoriesForEachCategory(categories:Category[], expenses:Expense[]):Category[]{
